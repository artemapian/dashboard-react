        ...
        <div className="flex flex-1 h-17 min-w-[260px] items-center justify-between px-4 py-2 border-1 border-blue-200 rounded-md text-[14px]">
            ...
            </div>
            {
                layout.find(item => item.i === text && item.key === widgetKey) ? (
                    ...
                    <button onClick={() => setLayout([{i: text, key: widgetKey, x: 0, y: 0, w: 1, h: 1}, ...layout])} className="items-center cursor-pointer hover:border-blue-400 h-6 w-10 hover:text-blue-400 text-black border-1 border-gray-300 rounded-md">
                        Add
                    </button>
                )
            }
